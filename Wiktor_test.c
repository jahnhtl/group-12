...
void adjustForCurve(int distanceLeft, int distanceRight) {
  ...
  if (difference > 0) {
    ...
    rightSpeed = motorSpeed - difference * 2; // Je größer die Differenz, desto mehr lenken nach rechts
  } else if (difference < 0) {
    // Das Hindernis ist näher am rechten Sensor, lenke nach links
    leftSpeed = motorSpeed + difference * 2; // Je größer die Differenz, desto mehr lenken nach links
  }
