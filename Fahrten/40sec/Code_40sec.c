...
long circle2 = 0;
int circleTarget2 = 500;

short intensityFactor;
...
void adjustForCurve(int distanceLeft, int distanceRight, int distanceFront) {
  // Initialisierung des Intensitätsfaktors
  float intensityFactor;

  // Überprüfen, ob der Frontsensor eine große Entfernung zur Wand feststellt
  if (distanceFront > 40) {
    intensityFactor = 200; // Passen Sie diesen Wert entsprechend Ihrer Anforderungen an
  } else {
    // Berechne den Kurvenradius basierend auf dem Durchschnitt der Seitenabstände
    float curveDiameter = (distanceLeft + distanceRight) / 2.0;
  
    // Berechne den Intensitätsfaktor basierend auf dem Kurvenradius
    intensityFactor = map(curveDiameter, 10, 70, 7, 120); // Passen Sie den Bereich der Kurvenradien und Intensitätsfaktoren nach Bedarf an
  }
