...
void loop() {
  ...
  if (isRunning) {
    ...
    if (distanceLeft > 35 || distanceRight > 35) {
      ...
      mode = 1;
	  displaySensorData(distanceLeft, distanceRight, distanceFront, mode, intensityFactor);
...
void adjustForCurve(int distanceLeft, int distanceRight) {
  // Berechne den Kurvenradius
  float curveRadius = (distanceLeft + distanceRight) / 2.0;

  // Berechne den Intensitätsfaktor basierend auf dem Kurvenradius
  float intensityFactor = map(curveRadius, 10, 70, 10, 20); // Passen Sie den Bereich der Kurvenradien und Intensitätsfaktoren nach Bedarf an
...
void displaySensorData(int distanceLeft, int distanceRight, int distanceFront, int mode, int intensityFactor) {
